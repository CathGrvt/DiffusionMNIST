from src.models import CNN, MIX_DDPM
...
def main():
    """
    The `main` function trains and evaluates two different deep learning models (MIX_DDPM and
    NAIVE_DDPM) on MNIST dataset, including visualization, loss plotting, FID score calculation using
    ...
    # Initialise mix model (both gaussian and custom) with default parameters
    gt = CNN(
        in_channels=1, expected_shape=(28, 28), n_hidden=(16, 32, 32, 16), act=nn.GELU
    )
    ddpm = MIX_DDPM(gt=gt, betas=(1e-4, 0.02), n_T=1000)
    optim = torch.optim.Adam(ddpm.parameters(), lr=2e-4)
    accelerator = Accelerator()
    ddpm, optim, mnist_dataloader = accelerator.prepare(ddpm, optim, mnist_dataloader)
    directory = create_directory("custom_mix_model")
